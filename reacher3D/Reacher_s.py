...
class ReacherEnv(gym.Env):
    ...
    def init_rod_template(self):
        self.rod_template_1 = np.matrix(
            np.zeros((int(math.ceil(self.resolution) + 10), int(math.ceil(self.resolution) + 10))))
        self.rod_template_1[30:-20 + int(math.ceil(self.resolution)),
        int(math.floor(((math.ceil(self.resolution) + 10) / 2.0) - (self.resolution * 0.15))):int(
            math.ceil(((math.ceil(self.resolution) + 10) / 2.0) + (self.resolution * 0.15)))] = 1
        self.rod_template_1 = self.rod_template_1.T

        self.rod_template_2 = np.matrix(
            np.zeros((int(math.ceil(self.resolution) + 10), int(math.ceil(self.resolution) + 10))))
        self.rod_template_2[30:-20 + int(math.ceil(self.resolution)),
        int(math.floor(((math.ceil(self.resolution) + 10) / 2.0) - (self.resolution * 0.1))):int(
            math.ceil(((math.ceil(self.resolution) + 10) / 2.0) + (self.resolution * 0.1)))] = 1
        self.rod_template_2 = self.rod_template_2.T

        self.rod_template_3 = np.matrix(
            np.zeros((int(math.ceil(self.resolution) + 10), int(math.ceil(self.resolution) + 10))))
        self.rod_template_3[30:-20 + int(math.ceil(self.resolution)),
        int(math.floor(((math.ceil(self.resolution) + 10) / 2.0) - (self.resolution * 0.05))):int(
            math.ceil(((math.ceil(self.resolution) + 10) / 2.0) + (self.resolution * 0.05)))] = 1
    ...
    def pd_control(self, jas, jvs, target_jas):
        ...
        return np.array(pos_gains * (np.matrix(angle_normalize(target_jas - jas))).T).flatten() - np.array(
            (damp_gains * np.matrix(jvs).T).T).flatten()
    ...
    def rand_target(self):
        ...
        self.invalid_target = rendering.make_invalid_target(self.invalidParams[0], self.invalidParams[1],
  ...
  """
    ...
    def _step(self, (jas, jvs, target_ja, torques)):
        if (self.controlMode == "POS"):
            jointAngles = np.array([self.j1.getAngle(), self.j2.getAngle(), self.j3.getAngle(), self.j4.getAngle(), self.j5.getAngle(), self.j6.getAngle(), self.j7.getAngle()])
            jointVelocities = np.array(
                [self.j1.getAngleRate(), self.j2.getAngleRate(), self.j3.getAngleRate(), self.j4.getAngleRate(), self.j5.getAngleRate(), self.j6.getAngleRate(), self.j7.getAngleRate()])
            ...
            self.j4.addTorque(output_torques[3])
            self.j5.addTorque(output_torques[4])
            self.j6.addTorque(output_torques[5])
            self.j7.addTorque(output_torques[6])
        if (self.controlMode == "POS-IMG"):
            ...
            self.j4.addTorque(torques[3])
            self.j5.addTorque(torques[4])
            self.j6.addTorque(torques[5])
            self.j7.addTorque(torques[6])
        ...
        if (self.controlMode == "TORQUE"):
            ...
            self.j6.addTorque(torques[3])
            self.j7.addTorque(torques[3])
    ...
    def _render(self, mode='human', close=False):
        ...
        if self.viewer is None:
            ...
            self.viewer.set_bounds(-self.spaceSize / 2.0, self.spaceSize / 2.0, -self.spaceSize / 2.0,
                                   self.spaceSize / 2.0)
        ...
        self.transform_link(self.body1, self.pole_transform11, self.pole_transform1, self.axle_transform12,
                            self.axle_transform1)
        self.transform_link(self.body2, self.pole_transform21, self.pole_transform2, self.axle_transform22,
                            self.axle_transform2)
        self.transform_link(self.body3, self.pole_transform31, self.pole_transform3, self.axle_transform32,
                            self.axle_transform3)
        self.transform_link(self.body4, self.pole_transform41, self.pole_transform4, self.axle_transform42,
                            self.axle_transform4)
        self.transform_link(self.body5, self.pole_transform51, self.pole_transform5, self.axle_transform52,
                            self.axle_transform5)
        self.transform_link(self.body6, self.pole_transform61, self.pole_transform6, self.axle_transform62,
                            self.axle_transform6)
        self.transform_link(self.body7, self.pole_transform71, self.pole_transform7, self.axle_transform72,
        ...
        self.ground_truth_joint_angles = np.array(
            [self.j1.getAngle(), self.j2.getAngle(), self.j3.getAngle(), self.j4.getAngle(), self.j5.getAngle(), self.j6.getAngle(), self.j7.getAngle()])
